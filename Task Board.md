---

kanban-plugin: board

---

## ## 🟣Backlog

- [ ] #### Props for PCG
	#alex
- [ ] #### Determine protagonist style
	#sivak
- [ ] #### Soundtrack 
	#egor


## ## 🔴To Do

- [ ] ##### Проработка пути игрока
	#alex
	
	> Детализация серпантина на локации монастыря
	
	❗Deadline → 08.03.2026
- [ ] ##### 3D моделирование комплексов монастыря
	#alex
	> Моделирование простых форм отдельных частей монастыря для прототипирования
	
	❗Deadline → 15.03.2026
- [ ] ##### Распределение мелких активностей на префабе
	#alex
	> Создание в 3D из базовых форм side-активностей и размещение из на префабе
	
	❗Deadline → 22.03.2026
- [ ] ##### Проработка архитектуры монастыря
	#russkikh 
	> Разработать концепт монастырского комплекса в соответствии с новым сеттингом
	
	❗Deadline → 22.03.2026
- [ ] ##### Проработка сеттинга
	#egor
	> Формализация основных элементов сеттинга и консультация. Дальнейшие правки
	
	❗Deadline → 01.03.2026
- [ ] ##### Система взаимодействий
	#egor
	> Реализация интерактивной системы с npc. 
	> Разработка процедурной трехактной системы квестов
	
	❗Deadline → 08.03.2026
- [ ] ##### Разработка системы случайного стриминга sub-levels 
	#egor
	> Написание Blueprint классов, отвечающих за случайное перемешивание подлокаций на основной локации
	
	❗Deadline → 15.03.2026
- [ ] ##### Проработка квестовой линии 1 на двух npc
	#polina 
	#egor
	> Проработка архетипов персонажей
	> Написание и формализация квеста в  Twine
	
	❗Deadline → 15.03.2026
- [ ] ##### Телосложение протагониста
	#sivak
	> Создание концепт-артов фигуры героя
	> Разработка телесных отличительных особенностей
	
	❗Deadline → 01.03.2026
- [ ] ##### Дизайн одежды
	#sivak
	> Создание концепт-артов стартового комплекта одежды
	
	❗Deadline → 15.03.2026
- [ ] ##### Портрет протагониста
	#sivak
	> Внесение правок в лицевовй портрет героя в соответствии с сеттингом
	
	❗Deadline → 22.03.2026
- [ ] ##### Трансформация механик
	#konstantin 
	> Переработка механик в соответствии с новым сеттингом
	
	❗Deadline → 01.03.2026
- [ ] ##### Механика угроз
	#konstantin 
	> Разработка механики отвечающей за трудности игрока в соответствии с сеттингом
	
	❗Deadline → 08.03.2026
- [ ] ##### Механика наград
	#konstantin 
	> Разработка механики отвечающей за дофамин игрока в соответствии с сеттингом
	
	❗Deadline → 15.03.2026


## ## 🟠 In Progress

- [ ] ##### Level-Design по концепту монастыря
	#alex
	
	> Формирование базового landscape и дизайна уровня
	
	❗Deadline → 01.03.2026
- [ ] #### Проработка Quest Line 1
	#egor 
	#polina
- [ ] #### Проработка сценария
	#egor
- [ ] #### Fooliage art  
	#alex
- [ ] #### Проработка архитектуры монастыря
	#russkikh  
	Deadline → 01.01.2026
- [ ] #### Проработка механики медитации
	#konstantin 
	Deadline → 15.02.2026
- [ ] #### Base body sketches 
	#sivak  
	Deadline → 15.02.2026


## ## 🟢Completed

- [ ] #### Tibet monastery POI base sketches
	initial broad exploration
	#russkikh  
	Deadline → 08.02.2026
	![[Tibet_Monastery_POI - sketch 1.png]]
- [ ] #### Доработка механик
	#konstantin 
	Deadline → 08.02.2026
- [ ] #### Цветокоррекция портрета протагониста. Рефактор костной структуры
	#sivak 
	Deadline → 08.02.2026
	![[Johan_base_face_colored.jpg]]
- [ ] #### Total game mechs loops
	#konstantin 
	#egor  
	Deadline → 01.02.2026
- [ ] #### Quest system rework
	#egor 
	#polina  
	Deadline → 07.02.2026
- [ ] #### Develop POIs distribution algorithm 
	#egor
	#alex 
	Deadline → 31.01.2026
	[[2. Prototype of basic terrain generation|note]]
- [ ] #### Johan base facial concepts
	#sivak  
	Deadline → <mark style="background: [[FF5582A6]];">24.01.2026</mark>
	![[Johan_base_face_sketches.jpg]]
- [ ] #### Tibet monastery mood board
	initial broad exploration
	#russkikh  
	#egor 
	Deadline → <mark style="background: [[FF5582A6]];">27.01.2026</mark>
- [ ] #### Rough POI sketches 
	expanding visual range
	#russkikh  
	Deadline → 24.01.2026
- [ ] #### Проработка предыстории протагониста
	#egor 
	#polina  
	Deadline → <mark style="background: [[FF5582A6]];">17.01.2026</mark>
- [ ] #### Чтение книг, генерация идей для hook механик
	#konstantin 
	Deadline → <mark style="background: [[FF5582A6]];">17.01.2026</mark>
- [ ] #### Migrate current C++ Classes to UE 5.6.1
	#egor
	Deadline → <mark style="background: [[FF5582A6]];">17.01.2026</mark>




%% kanban:settings
```
{"kanban-plugin":"board","list-collapse":[true,false,false,false],"show-checkboxes":false,"new-card-insertion-method":"append","hide-card-count":false,"move-tags":true,"tag-colors":[{"tagKey":"","color":"","backgroundColor":""}],"move-dates":true,"date-format":"DD-MM-YYYY","date-display-format":"DD-MM-YYYY","date-colors":[],"move-task-metadata":true,"show-view-as-markdown":false,"inline-metadata-position":"body","date-picker-week-start":1,"lane-width":300}
```
%%